...
```powershell
# Build that static site for GitHub Pages (base path /Moirai, backend forced to wasm, 404.html fallback)
yarn build:pages
```
...
- **`Core/ExecuteContext.cs`** drives time (`PassYears`). The clock (`Ctx.Year`) and `Time.year` are one fact: `Database.SetProperty` moves the clock whenever `Time.year` is written, which is what stamps `@start` records with the year the story set (764 in w.sg) instead of 0, and changesets take their year when they close. It also holds the runtime value stack (`SetArgument`/`Argument`, scoped via `RunScope`) and the RNG (`Pcg32`, seeded — simulation is deterministic per seed).
...
- **`ClientAppSvelte/`**: SvelteKit SPA (`adapter-static` + `ssr = false`, so it is a standalone SPA that can be served by anything). `src/lib/api.ts` declares `MoiraiApi` — the transport-independent client surface — and `src/lib/backend.ts` picks an implementation: `signalr-api.ts` (the .NET host) or `wasm-api.ts` (the engine in the browser), chosen by `?backend=wasm` then `VITE_MOIRAI_BACKEND`. The choice is **remembered in `sessionStorage`**, which is load-bearing: the nav links do real navigation, so every page change is a fresh document with no query string, and without stickiness clicking "World" after opening `?backend=wasm` silently drops you onto the other backend — invisible whenever a server happens to be running. `src/lib/connection.ts` no longer knows about either; it holds the `moiraiStore` Svelte store the whole UI subscribes to. `wasm-api.test.ts` drives a fake engine to cover what only the host can get wrong: that a long pass is split into chunks and yields between them, that records arrive *during* a pass rather than all at the end, that cancellation stops at a chunk boundary, and that the feed backlog stops the first tick (the reset notice plus every `@start` record) being lost between the world existing and the store subscribing. `backend.test.ts` pins the sticky backend choice.
  ...
  **`$lib/settled-year`** is why a pass does not thrash the pages. Five places re-query the world when the year changes — rule coverage, the world overview and its series, an entity's changesets, a family tree, a biography — and with the in-browser engine those queries are synchronous work on the thread that is trying to paint. `settledYear` reports the year only once it stops moving: a change is held for 400 ms of quiet, never longer than 1.5 s (so a long pass still shows progress rather than freezing), and flushed at once when a pass ends. Measured over a 400-year pass: 37 year updates became 2 refetches. It reports nothing until a backend exists, which gives each page exactly one trigger at startup instead of one for the empty world and another for the real one. **No page or component names a transport** — they call `conn.getBiography(...)` and get whichever backend loaded. `src/routes/` are the pages (`/` is a placeholder home view — it used to redirect to `records` — plus `records`, `life`, `changesets`, `query`, `family`, `world`, `rules`, `story`); `src/components/` the widgets. Chart and meter colours live in the `.viz-root` token block at the bottom of `src/app.css` — every value is a step of the active Skeleton theme, and the steps were validated against the theme's surface, so **re-validate them if the theme changes**. Page logic that is worth testing lives in `$lib` (e.g. `coverage.ts`, `chart.ts`) with a vitest file beside it, not in the `.svelte` file. The **Life** page (`GetBiography`) is the one place the three per-entity sources meet: records mentioning it, changesets touching it, and its family tree. Records and changes interleave by `(year, changesetId)` — a record carries the id of the changeset that produced it, so that ordering is causal — with a **stable** sort, because an event and the triggers it fires share a changeset id. `components/LineChart.svelte` is a hand-rolled SVG line chart (no chart library): it measures its own width so a 2px line is 2px at any size, and carries a crosshair, a direct end-label, an `aria-label` summary and an optional table view.

  **The Story page edits the `.sg` in the browser**, and exists only where `conn.story` is non-null — a capability on `MoiraiApi`, not a backend check, because the server's story is a file its watcher owns while the in-browser engine's is a string it can be handed another of. Squiggles are the *engine's*: every pause in typing runs the real parser over the text through `ValidateStory`, so the editor can never disagree with the thing that builds the world. `$lib/moirai-language.ts` is therefore colour only, and its keyword list is checked against `MoiraiTokenizer.Keywords` by a vitest that reads the C# file — the same guard `SyntaxHighlightingDriftTests` gives the LSP. `$lib/diagnostics.ts` is the one place the engine's **1-based line, 0-based column** meets CodeMirror's absolute offsets; it clamps, because validation is debounced and a diagnostic routinely describes text that has since been deleted — an out-of-range line throws inside `doc.line`, and a linter that throws reports *nothing*, which looks exactly like a clean story. CodeMirror is imported dynamically from `$lib/story-editor.ts`, so its ~320 KB lands in the `/story` route chunk and no other page fetches it. The draft is kept in `localStorage` — it is your work, and closing the tab should not lose it — while the world's own identity lives in the URL (see below). **The mount effect writes `view` and must never read it** — an effect that reads the state it assigns re-runs on its own assignment; the version that did stacked 347 editors in the page and left every button disabled.

  **The URL is the world's address.** `?seed=&year=` in the query string is the whole identity of a world; `$lib/world-address.ts` reads and writes it and `backend.ts` is the only caller that reads it at boot. Four consequences worth knowing separately:
  - **A bare link opens on two centuries of history**, not on an empty world (`WasmApi.openTo`, about 1.5 s including the runtime boot). A world at its start year holds no records at all, so arriving there asks a visitor to work out that they must press a button before anything exists.
  - **The nav reads `window.location.search`, never `$page.url.search`.** SvelteKit's `replaceState` updates the browser's URL and *deliberately* leaves `page.url` on the last real navigation — it stores the old href in the history entry and sets only `page.state` (see `replaceState` in kit's `client.js`). A derived from it goes stale the moment the year is written, and a tab click then navigates to a year the world has already passed, sending you backwards in time. Caught in a browser, not by a test.
  - **The story does not live in the address bar.** It is 37 KB, and a link that long is unreadable. It rides only in a link built on demand by the home page's Copy button, deflated into the fragment (~13 KB for an edited `w.sg`), and only when it differs from the shipped story. On arrival it is stored as the local draft, which is why the app's own navigations can drop the fragment.
  - **Which backend gets a URL is a capability, not a check**: `MoiraiApi.worldInPage`. The server has one shared world of its own, and a link naming a seed and a year would describe nothing the recipient could see.
...
- **`SetStory` / `ValidateStory` / `GetStory`** are how a viewer edits the world's source. `SetStory` validates first and does nothing if the story does not parse, so a broken edit costs the world nothing; a story that parses but throws on the way up puts the old text back. `ValidateStory` **saves and restores `Database.Instance`** — a mutable static that live simulation code reads (`Changeset` clones an entity through it), so a throwaway parse would otherwise leave the running world describing its entities with another story's types.
...
- **A world survives a page load, and travels.** A world is entirely determined by its story, seed and year, so nothing has to be serialized to rebuild it — and those three fit in a URL, which is where they live now (`$lib/world-address.ts`). That is what makes a world something you can send someone; `wasm-api.ts` is handed the three and fast-forwards in its usual chunks. Verified byte-identical — same entity, record and changeset counts — at roughly 170 years per second, which is also the trick the server's `_targetYears` uses after a story file changes on disk.
...
**Not available in the browser:** the DAP debugger, `.sg` hot reload, and the console profile report. All three stay server-only, which is why both backends coexist. The traffic runs the other way too: **editing the story is browser-only** (the Story page), because the server's story is a file its own watcher already owns.

### GitHub Pages — the site at `theor.github.io/Moirai`
`.github/workflows/pages.yml` publishes the WebAssembly build on every push to `main` (and on demand). It is a separate workflow from `dotnet.yml` because it needs the `wasm-tools`/`wasm-experimental` workloads that `Moirai.Wasm` is kept out of the solution to avoid forcing on everyone — and because a deployment has no business gating a pull request. It runs `yarn wasm:build` → `wasm:smoke` → tests/check/lint → `build:pages`, then `upload-pages-artifact`/`deploy-pages`.

Four things make a Pages deployment different from `yarn build`, and **every one of them fails silently**:
- **A project site is served under the repository name**, so the whole app needs a `/Moirai` prefix. `kit.paths.base` reads `BASE_PATH` (empty everywhere else, so dev and the .NET host are untouched), and the three URLs the app hardcodes go through it: `wasm-api.ts` builds `${base}/w.sg` and `${base}/_framework/main.js` — both files live in `static/`, which no bundler rewrites — and the app bar's icon uses `asset()`. Vite rewrites the font URLs in `app.css` to be relative on its own. **A redirect is not resolved for you**: the root route's `redirect(307, '/records')` walked straight out of the site, which is how this was caught.
- **Pages has no SPA rewrite.** A deep link is a file that does not exist, so the server answers with `404.html` — `build:pages` copies `index.html` there, which turns Pages' 404 handling into the app's router. The page loads with an HTTP 404 status, which nothing observes.
- **`static/.nojekyll`**, because `_app` and `_framework` start with an underscore. The artifact-based deploy used here does not run Jekyll, so this is insurance against ever switching to a branch deploy — where it would silently serve a site with no runtime and no JavaScript.
- **The backend has to be pinned**: `build:pages` sets `VITE_MOIRAI_BACKEND=wasm`, since a site with no server that tries SignalR just hangs.

Verified locally by serving `build/` behind a `/Moirai` prefix with a stand-in that mimics Pages' 404 behaviour, and driving it: root, deep link, engine boot, a pass, and a tab switch rebuilding the world from its URL. Switching to a custom domain later is dropping `BASE_PATH` from the workflow and committing a `static/CNAME`.
...
- **`TestProject1/`** — main NUnit suite for the engine/DSL (`EventTests`, `FilterTests`, `MatchTests`, `ParsingTests`, …). `WorldSessionTests` drives `Moirai.Api.WorldSession` the way a viewer does, with no server standing up — it is the safety net for the logic both hosts share, and it pins the wire format the client's hand-written TypeScript depends on. Plus the parser suites left over from the ANTLR→Superpower migration: `GrammarStructuralTests` (the corpus parses at all), `GrammarRuleTests` (one micro-fixture per grammar rule, so a regression localises to a rule instead of "somewhere in a 1000-line file"), `ChunkedErrorRecoveryTests` (one broken definition must not lose the rest of the file), and `TokenizerTests`/`ParserGoldenTests`. The last two are snapshot suites over the `.sg` corpus — token stream, printed `Database`, and the 50-year record history — and replace the differential suites that used to compare against the frozen ANTLR parser. Re-bless with `UPDATE_GOLDENS=1 dotnet test TestProject1`. The `.sg` files in `MoiraiCli/` (`w.sg`, `test.sg`, `space.sg`) are sample stories used for manual runs.
  - **Step-through tests go through `DebugRun`** (`TestProject1/DebugRun.cs`), which runs a story under a `DebugSession` on a worker thread. Two rules live there rather than in each test, because both were got wrong by hand and both fail only on a busy machine. **Breakpoints are an argument to `Start`, set before the thread begins**: a story runs its first year in microseconds, so arming a breakpoint afterwards is a race the run wins whenever the runner is loaded, and the test then waits out its timeout for a stop that can no longer happen — that is exactly how `StepIntoFunction` passed locally for months and failed on GitHub Actions. And **`Drain` never gives up on a slow worker**: it continues on a tick until the thread finishes, which is safe because `Suspend` resets its gate before raising `Stopped`, so an early continue cannot skip a stop that has not happened yet. The loop it replaced waited a second for the next stop and broke out if none came, leaving the simulation suspended for good.
