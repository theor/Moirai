...
Verified locally by serving `build/` behind a `/Moirai` prefix with a stand-in that mimics Pages' 404 behaviour, and driving it: root, deep link, engine boot, a pass, and a tab switch rebuilding the world from `sessionStorage`. Switching to a custom domain later is dropping `BASE_PATH` from the workflow and committing a `static/CNAME`.
