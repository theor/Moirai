...
- **`PublishTrimmed=false`**: the query page dumps the parsed AST with `JsonSerializer.Serialize(e, e.GetType(), …)`, and trimming would strip the members it walks — failing as an empty JSON object rather than a build error.
...
- **Trimming**: `TrimMode=partial` trims the framework but not us — untrimmed the bundle is 25 MB, of which ~440 KB is Moirai; trimmed it is 5 MB. `partial` is the load-bearing part: two things reflect over our own types (the query page's AST dump, and the DTOs through reflection-based `System.Text.Json`) and would fail *silently* if our assemblies were trimmed.
